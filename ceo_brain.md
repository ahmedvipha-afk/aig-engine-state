...
| 5 | Add MBV as 3rd strategy + validate across 3 markets | ⏳ PENDING | `aig/strategy_mbv.py` written, frozen in config, registered in Trial Budget (+3 trials → n_trials_registered=9), run on US+UAE+Crypto | DEFERRED per audit Path 2 recommendation: ship Path 1 (US Divergence paper-forward) full deployment cycle FIRST, then add MBV. Will activate once paper-forward history reaches the audit's "one full cycle observed" threshold. |
...
**Currently working:** Items 5 (MBV — DEFERRED per audit) and 6 (paper-forward history accumulation — PASSIVE wait). Items 1, 2, 3, 4, 7, 8 done. Sprint convergence near; Item 5 reactivates after Item 6 closes.
