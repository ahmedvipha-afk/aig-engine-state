...
def _strategy_signals(name: str, df: pd.DataFrame) -> pd.DataFrame:
    ...
    if name == "roc":
        ...
        return signals(df)
    if name == "vcb":
        from aig.strategy_vcb import signals
        return signals(df)
    if name == "hat":
    ...
    if name == "pmr":
        ...
        return signals(df)
    if name == "str":
        from aig.strategy_str import signals
        return signals(df)
...
def _stop_distance(name: str, row) -> float:
    ...
    if name == "vcb":
        from config import VCB_STOP_ATR_MULT
        return VCB_STOP_ATR_MULT * row["atr"]
    if name == "hat":
    ...
    if name == "str":
        from config import STR_STOP_ATR_MULT
        return STR_STOP_ATR_MULT * row["atr"]
